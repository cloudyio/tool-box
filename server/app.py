...
def unshorten():
    ...
    shortcode = path_segments[path_segments.index('s') + 1]
    if shortener.find_one({'short': url}):
        return jsonify({'success': True, 'url': shortener.find_one({'short': shortcode})['url']})
    else:
        return jsonify({'success': False, 'message': 'URL not found'})
...
def download():
    ...
    if os.path.exists('./downloads/' + yt.video_id + '.mp4'):
        get_download = 'https://cloudys.tech/api/download/' + yt.video_id
