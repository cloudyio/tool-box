

<div class="main">
    <div class="text-white bg-bgdark text-center flex flex-col mt-10">
        <h1 class="text-4xl font-bold mb-4">Toolbox</h1>
        <h3 class="text-lg mb-6">Made By Cloudy</h3>
    </div>
    
    <div class="bg-cover bg-bgdark flex justify-center flex-row h-screen">
        <div class="flex flex-wrap justify-center">
            <a href="/ytdownloader" class="bg-white dark:bg-slate-800 rounded-lg px-6 py-8 ring-1 sm:w-full ring-slate-900/5 shadow-xl md:w-1/5 md:h-2/6 m-6">
            <div
                class="" >
                <div>
                    <span
                        class="inline-flex items-center justify-center p-2 bg-indigo-500 rounded-md shadow-lg">
                        <svg
                            class="h-6 w-6 text-white"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            aria-hidden="true"><!-- ... --></svg>
                    </span>
                </div>
                <h3
                    class="text-slate-900 dark:text-white mt-5 text-base font-medium tracking-tight">
                    Youtube Video Downloader
                </h3>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm">
                    Allows you to download any youtube video quickly without youtube premium.
                </p>
            </div>
        </a>
        
        <a href="/shortener" class="bg-white dark:bg-slate-800 rounded-lg px-6 py-8 ring-1 sm:w-full ring-slate-900/5 shadow-xl md:w-1/5 md:h-2/6 m-6">
            <div
                >
                <div>
                    <span
                        class="inline-flex items-center justify-center p-2 bg-indigo-500 rounded-md shadow-lg">
                        <svg
                            class="h-6 w-6 text-white"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            aria-hidden="true"><!-- ... --></svg>
                    </span>
                </div>
                <h3
                    class="text-slate-900 dark:text-white mt-5 text-base font-medium tracking-tight">
                    Url Shortener
                </h3>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm flex-wrap ">
                    Url too long? This tool shortens long links to make them neater.
                </p>
            </div>
        </a>

        <a href="/unshortener" class="bg-white dark:bg-slate-800 rounded-lg px-6 py-8 ring-1 sm:w-full ring-slate-900/5 shadow-xl md:w-1/5 md:h-2/6 m-6">
            <div
                >
                <div>
                    <span
                        class="inline-flex items-center justify-center p-2 bg-indigo-500 rounded-md shadow-lg">
                        <svg
                            class="h-6 w-6 text-white"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            aria-hidden="true"><!-- ... --></svg>
                    </span>
                </div>
                <h3
                    class="text-slate-900 dark:text-white mt-5 text-base font-medium tracking-tight">
                    Url unshortener
                </h3>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm flex-wrap ">
                    One of my urls looking suspicious? This tool will unshorten any of my urls to see where they lead.
                </p>
            </div>
        </a>
        
        <a href="/" class="bg-white dark:bg-slate-800 rounded-lg px-6 py-8 ring-1 sm:w-full ring-slate-900/5 shadow-xl md:w-1/5 md:h-2/6 m-6">

            <div
                >
                <div>
                    <span
                        class="inline-flex items-center justify-center p-2 bg-indigo-500 rounded-md shadow-lg">
                        <svg
                            class="h-6 w-6 text-white"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            aria-hidden="true"><!-- ... --></svg>
                    </span>
                </div>
                <h3
                    class="text-slate-900 dark:text-white mt-5 text-base font-medium tracking-tight">
                    Many more tools to come
                </h3>
                <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm flex-wrap ">
                    Stay tuned for more useful tools that you may need in the future. Have an idea? Contact me and I will make it happen.
                </p>
            </div>
        </a>
            
        </div>
        
    </div>
</div>
